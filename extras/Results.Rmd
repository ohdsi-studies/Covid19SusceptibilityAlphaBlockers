...
---
```{r, echo=FALSE}
outputTableAsPng <- TRUE # If TRUE, output each table individually as a png file.
```
...
databaseIdLabels <- c("SIDIAP", "VA", "CUIMC", "OpenClaims", "Optum DOD", "Optum EHR")
names(databaseIdLabels) <- databaseIds
...
if (outputTableAsPng) {
  table_name <- "population_size_exposure_time_table"
  texfile_name <- paste0(table_name,".tex")
  sink(file=texfile_name)
}
...
if (outputTableAsPng) {
  sink()
  tools::texi2dvi(file=texfile_name)
  cmd <- paste("dvipng -T tight", shQuote(paste0(table_name, ".dvi")))
  invisible(system(cmd))
  cleaner <- c(".tex", ".aux", ".log", ".dvi")
  invisible(file.remove(paste0(table_name, cleaner, sep="")))
}
```
...
png(file = "allHazardRatioWithCI.png", height = 10, width = 8, 
    units = 'in', bg = "transparent", res = 600)
