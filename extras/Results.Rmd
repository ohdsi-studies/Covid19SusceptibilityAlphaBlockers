...
makeSampleSizeTable <- function(databaseId, 
  ...
  cmResults <- cmResults %>% filter(analysisId %in% analysisIdToKeep, outcomeId == covidDiagnosisId)
...
printBaselineCharacteristicTable <- function (databaseIds, dataDirs, databaseIdLabels, header, footer) {
  ...
  tableOrder$id <- NULL
  table_tmp <- full_join(table_tmp, tableOrder, by = "join_id") %>% arrange(order) %>% 
    filter(order < 300) %>%
    mutate(id = newNames) %>%
