...
makeSampleSizeTable <- function(databaseId, 
  ...
  names(outcomeIds) <- c("Diagnosis", "Hospitalization", "IntensiveService")
  cmResults <- cmResults %>% filter(
    analysisId %in% analysisIdToKeep, 
    outcomeId %in% outcomeIds 
  )
  
  recodeList <- names(outcomeIds)
  names(recodeList) <- sapply(outcomeIds, toString)
  cmResults <- cmResults %>% mutate(outcomeId = recode(outcomeId, !!!recodeList))
  cmResults <- cmResults %>% rename(target = targetOutcomes, comparator = comparatorOutcomes)
  cmResults <- cmResults %>% pivot_wider(names_from = outcomeId, values_from = c(target, comparator), names_sep = "")
  ...
  table <- rbind(
    ...
    select(targetId, comparatorId, databaseId, targetSubjects, comparatorSubjects, targetYears, comparatorYears, targetDiagnosis, targetHospitalization, targetIntensiveService, comparatorDiagnosis, comparatorHospitalization, comparatorIntensiveService, mdrr, order, databaseOrder, analysisId)
  ...
  for (outcome in names(outcomeIds)) {
    targetOutcome <- paste0("target", outcome)
    table[[targetOutcome]] <- gsub("^-", "<", formatC(table[[targetOutcome]], big.mark = ",", format = "d"))
    comparatorOutcome <- paste0("comparator", outcome)
    table[[comparatorOutcome]] <- gsub("^-", "<", formatC(table[[comparatorOutcome]], big.mark = ",", format = "d"))
  }
...
addAnalysisInfo <- function (analysisId) {
  info <- rep("(matched)", length(analysisId))
  info[analysisId == 5] <- "(stratified)"
  return(info)
}
table <- table %>% mutate(databaseId = paste(databaseId, addAnalysisInfo(analysisId)))
```
...
table <- table %>% select(-analysisId)
```

% Table begins: population_size_exposure_time_covid_diagnosis_table
\begin{table}[H]
...
print(xtable(table, format = "latex"),
      ...
      add.to.row = list(
        pos = as.list(c(0) * 3),
        command = c(
          "                       \\multicolumn{9}{l}{Alpha-1 blocker vs 5ARI / PDE5} \\\\"
...
% Table begins: relative_risk_covid_diagnosis_table
\begin{table}[H]
...
print(xtable(wideTable %>% filter(outcomeId == covidDiagnosisId) %>% 
 ...
 add.to.row = list(
        ...
        command = c(
          "                       \\multicolumn{6}{l}{Alpha-1 blocker vs 5ARI / PDE5} \\\\"
...
% Table begins: relative_risk_covid_diagnosis_stratified_and_matched_analysis_table
...
print(xtable(annotatedWideTable %>% filter(outcomeId == covidDiagnosisId) %>% 
 ...
 add.to.row = list(
        pos = as.list(c(0) * numDb),
        command = c(
          "                       \\multicolumn{10}{l}{Alpha-1 blocker vs 5ARI / PDE5} \\\\"
...
% Table begins: relative_risk_covid_hospitalization_stratified_and_matched_analysis_table
\definecolor{light-gray}{gray}{0.50}
\begin{table}[H]
 \caption{Relative risk of COVID-19 hospitalization for Alpha-1 blocker and 5ARI / PDE5 prevalent-users. We report calibrated hazard ratios (HRs) and their 95\% confidence intervals (CIs) across data sources. Grayed out entries do not pass study diagnostics.}
  \rowcolors{2}{gray!6}{white}
  \centering{
  \begin{tabular}{p{-2em}p{-2em}lrrrc@{}rrr}
    \hiderowcolors
    \toprule
    & & & \multicolumn{7}{c}{Hospitalization} \\
    & & & \multicolumn{3}{c}{PS-stratified} & &  \multicolumn{3}{c}{PS-matched} \\
     \cmidrule(lr){4-6} \cmidrule(lr){8-10}
     & 
    & & \multicolumn{1}{c}{HR} & \multicolumn{1}{c}{95\% CI} & \multicolumn{1}{c}{$p$} &
...
% Table begins: relative_risk_covid_intensive_services_stratified_and_matched_analysis_table
\definecolor{light-gray}{gray}{0.50}
\begin{table}[H]
 \caption{Relative risk of COVID-19 hospitalization requiring intensive services for Alpha-1 blocker and 5ARI / PDE5 prevalent-users. We report calibrated hazard ratios (HRs) and their 95\% confidence intervals (CIs) across data sources. Grayed out entries do not pass study diagnostics.}
  \rowcolors{2}{gray!6}{white}
  \centering{
  \begin{tabular}{p{-2em}p{-2em}lrrrc@{}rrr}
    \hiderowcolors
    \toprule
    & & & \multicolumn{7}{c}{Intensive services} \\
    & & & \multicolumn{3}{c}{PS-stratified} & &  \multicolumn{3}{c}{PS-matched} \\
     \cmidrule(lr){4-6} \cmidrule(lr){8-10}
     & 
    & & \multicolumn{1}{c}{HR} & \multicolumn{1}{c}{95\% CI} & \multicolumn{1}{c}{$p$} &
