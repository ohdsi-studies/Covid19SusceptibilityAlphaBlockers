...
alphaBlockerId <- cohorts %>% filter(shinyName == "Alpha-1 blockers") %>% select(cohortId) %>% pull()
fiveAlphaId <- cohorts %>% filter(shinyName == "5-alpha inhibitors") %>% select(cohortId) %>% pull()
...
prettyNames <- data.frame(
  ...
  cohortName = c("Alpha blocker", "5-alpha RI"))
...
sidiapDir <- file.path(studyFolder, "SIDIAP")
cuimcDir <- file.path(studyFolder, "CUIMC")
vaDir <- file.path(studyFolder, "VA-OMOP")
...
makeSampleSizeTable <- function(databaseId, 
  ...
  table <- rbind(
    cmResults %>% filter(targetId == alphaBlockerId, comparatorId == fiveAlphaId) %>% mutate(targetId = "Alpha blocker", comparatorId = "5-alpha RI", order = 1)
...
print(xtable(table, format = "latex"),
      ...
      add.to.row = list(
        pos = as.list(c(0) * 3),
        command = c(
          "                       \\multicolumn{9}{l}{Alpha blocker vs 5-alpha RI} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\"
...
```{r, echo=TRUE}
countText(targetName = "Alpha blocker", comparatorName = "5-alpha RI")
```
...
#}
 ...
 # tmp <- makeCharacteristicsTable(alphaBlockerId, fiveAlphaId, 
 #                          databaseIds = c("SIDIAP", "CUIMC"),
 #                          dataFolders = c(dataFolder = file.path(sidiapDir, "shinyData"),
 #                                          dataFolder = file.path(cuimcDir, "shinyData")))

# makeCharacteristicsTable(alphaBlockerId, fiveAlphaId,   databaseId = "SIDIAP", dataFolder = file.path(sidiapDir, "shinyData"))
# makeCharacteristicsTable(alphaBlockerId, fiveAlphaId,   databaseId = "CUIMC", dataFolder = file.path(cuimcDir, "shinyData"))
...
print(xtable(wideTable %>% filter(outcomeId == covidNarrowId) %>% 
 ...
 add.to.row = list(
        ...
        command = c(
          "                       \\multicolumn{6}{l}{Alpha blocker vs 5-alpha RI} \\\\ & \\multicolumn{5}{l}{Monotherapy}  \\\\"
...
print(xtable(annotatedWideTable %>% filter(outcomeId == covidNarrowId) %>% 
 ...
 add.to.row = list(
        pos = as.list(c(0) * numDb),
        command = c(
          "                       \\multicolumn{10}{l}{Alpha blocker vs 5-alpha RI} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\"
...
cohortNames <- c("Alpha blocker vs 5-alpha RI")
...
saveAllBalancePlots(getAllBalancePlots(alphaBlockerId, fiveAlphaId, limits = c(0, 0.6), yLimits = c(0, 0.2)),
